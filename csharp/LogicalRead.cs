...
namespace ParquetSharp
{
        ...
        where TPhysical : unmanaged
    {
        ...
        private static void ConvertDateTimeMicros(ReadOnlySpan<long> source, Span<DateTime> destination, bool isAdjustedToUtc)
        {
            for (int i = 0; i != destination.Length; ++i)
            {
                destination[i] = LogicalRead.ToDateTimeMicros(source[i], isAdjustedToUtc);
            }
        }
        ...
        private static void ConvertDateTimeMicros(ReadOnlySpan<long> source, ReadOnlySpan<short> defLevels, Span<DateTime?> destination, short nullLevel, bool isAdjustedToUtc)
        {
            for (int i = 0, src = 0; i != destination.Length; ++i)
            {
                destination[i] = defLevels[i] == nullLevel ? default(DateTime?) : LogicalRead.ToDateTimeMicros(source[src++], isAdjustedToUtc);
            }
        }
        ...
        private static void ConvertDateTimeMillis(ReadOnlySpan<long> source, Span<DateTime> destination, bool isAdjustedToUtc)
        {
            for (int i = 0; i != destination.Length; ++i)
            {
                destination[i] = LogicalRead.ToDateTimeMillis(source[i], isAdjustedToUtc);
            }
        }
        ...
        private static void ConvertDateTimeMillis(ReadOnlySpan<long> source, ReadOnlySpan<short> defLevels, Span<DateTime?> destination, short nullLevel, bool isAdjustedToUtc)
        {
            for (int i = 0, src = 0; i != destination.Length; ++i)
            {
                destination[i] = defLevels[i] == nullLevel ? default(DateTime?) : LogicalRead.ToDateTimeMillis(source[src++], isAdjustedToUtc);
            }
        }
    ...
    internal static class LogicalRead
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static DateTime ToDateTimeMicros(long source, bool isAdjustedToUtc)
        {
            return new DateTime(DateTimeOffset + source * (TimeSpan.TicksPerMillisecond / 1000), isAdjustedToUtc ? DateTimeKind.Utc : DateTimeKind.Unspecified);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static DateTime ToDateTimeMillis(long source, bool isAdjustedToUtc)
        {
            return new DateTime(DateTimeOffset + source * TimeSpan.TicksPerMillisecond, isAdjustedToUtc ? DateTimeKind.Utc : DateTimeKind.Unspecified);
        }
